...
* **Core File Modifications**:
  * 📄 `src/components/common/ThemeToggle.tsx`:
    - Tối ưu hóa kích thước chữ và khoảng cách trong bộ chuyển sáng/tối: đổi size chữ `text-xs` (12px) thành `text-[11px]`, đổi `font-black` thành `font-extrabold` giúp chữ mảnh và thanh thoát hơn, đổi `tracking-wider` thành `tracking-wide` để co các ký tự khít lại, đồng thời giảm khoảng cách `gap-2` giữa icon và text thành `gap-1.5`.
  * 📄 `src/app/hq/hq-dashboard-client.tsx`:
    ...
    - Điều chỉnh thẻ bọc `ThemeToggle` trên header trang HQ rộng ra từ `w-28` -> `w-36` -> `w-40` (160px) để đảm bảo không gian hiển thị rộng rãi, loại bỏ hoàn toàn hiện tượng nút Refresh đè lên chữ.
  * 📄 `src/app/page.tsx`:
...
### 🚨 Troubleshooting
> 🐛 **Problem Encountered (ThemeToggle Text Truncation on PC & Mobile)**: Chữ "TỐI" bị cắt mất chữ "I" và bị nút Refresh đè lên trên giao diện HQ do độ rộng container bọc `w-36` quá hẹp và font chữ in hoa dãn chữ `tracking-wider` của component chiếm quá nhiều không gian.
> 💡 **Solution**: Tăng chiều rộng container bọc lên `w-40` (160px). Đồng thời, trong component `ThemeToggle.tsx`, tối ưu font chữ từ `font-black text-xs tracking-wider gap-2` thành `font-extrabold text-[11px] tracking-wide gap-1.5` giúp nút hiển thị tinh gọn, sang trọng và không bao giờ bị cắt chữ trên mọi thiết bị.
