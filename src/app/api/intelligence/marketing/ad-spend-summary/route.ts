 ...
 * 
 * Example:
 * GET /api/intelligence/marketing/ad-spend-summary?tenantId=abc-123&period=month
 */

import { NextRequest, NextResponse } from 'next/server';
import { getAdSpendSummary } from '@/services/intelligence/marketing/queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getAdSpendSummary({
      tenantId,
      dateRange,
      platforms,
    });
    ...
    console.error('[API] Ad Spend Summary error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    
    return NextResponse.json(
      { error: 'Failed to get ad spend summary', details: errorMessage },
      { status: 500 }
    );
  }
}
