/**
 * API Route: Session Analytics
 * 
 * GET /api/intelligence/operational/session-analytics
 * 
 * Query params:
 ...
 * 
 * Returns: SessionAnalytics[] with metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getSessionAnalytics(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Session Analytics error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get session analytics', details: errorMessage },
      { status: 500 }
    );
  }
}
