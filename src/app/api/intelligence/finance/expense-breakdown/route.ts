 ...
 * 
 * Returns: ExpenseBreakdown with cache metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getExpenseBreakdown(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Expense breakdown error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get expense breakdown', details: errorMessage },
      { status: 500 }
    );
  }
}
