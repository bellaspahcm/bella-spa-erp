 ...
 * 
 * Returns: ProfitabilityTrends with cache metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getProfitabilityTrends(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Profitability trends error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get profitability trends', details: errorMessage },
      { status: 500 }
    );
  }
}
