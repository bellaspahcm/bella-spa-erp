/**
 * API Route: Operational Efficiency
 * 
 * GET /api/intelligence/executive/operational-efficiency
 * 
 * Query params:
 ...
 * 
 * Returns: OperationalEfficiency with metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getOperationalEfficiency(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Operational Efficiency error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get operational efficiency', details: errorMessage },
      { status: 500 }
    );
  }
}
