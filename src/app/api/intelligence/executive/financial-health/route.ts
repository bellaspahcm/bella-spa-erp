/**
 * API Route: Financial Health
 * 
 * GET /api/intelligence/executive/financial-health
 * 
 * Query params:
 ...
 * 
 * Returns: FinancialHealth with metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getFinancialHealth(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Financial Health error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get financial health', details: errorMessage },
      { status: 500 }
    );
  }
}
