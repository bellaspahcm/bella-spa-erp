/**
 * API Route: Customer Metrics
 * 
 * GET /api/intelligence/executive/customer-metrics
 * 
 * Query params:
 ...
 * 
 * Returns: CustomerMetrics with metadata
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await service.getCustomerMetrics(tenantId, dateRange);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] Customer Metrics error:', error);

    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to get customer metrics', details: errorMessage },
      { status: 500 }
    );
  }
}
