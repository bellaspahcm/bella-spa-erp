      ...
      <div className="px-6 -mt-12 relative z-20">
         <div className="flex items-end justify-center gap-2 mb-8">
            ...
            {leaderboard[1] && (
               <div className="flex flex-col items-center flex-1">
                  ...
                  <span className="text-[8px] font-black text-slate-800 dark:text-[#D4C5B6] uppercase">{leaderboard[1].sessions ?? 0} ca</span>
            ...
            {leaderboard[2] && (
               <div className="flex flex-col items-center flex-1">
                  ...
                  <span className="text-[8px] font-black text-slate-800 dark:text-[#D4C5B6] uppercase">{leaderboard[2].sessions ?? 0} ca</span>
         ...
         <div className="bg-white dark:bg-[#1C1B19] rounded-[40px] shadow-sm border border-slate-100 dark:border-[#3E3A35] overflow-hidden">
               ...
               return (
                 <div key={item.ktv_id} className="flex items-center justify-between p-5 border-b border-slate-50 dark:border-[#2E2B27] last:border-0">
                    <div className="flex items-center gap-4">
                       <span className="text-xs font-black text-slate-600 dark:text-[#D4C5B6] w-4">{index + 4}</span>
                       ...
                       <div>
                          ...
                          <p className="text-[10px] text-slate-800 dark:text-[#D4C5B6] font-medium">Hạng {item.rank} • {item.sessions ?? 0} ca làm</p>
                       </div>
                    </div>
                    <div className="text-right" title={ratingTooltip}>
                       <div className={`flex items-center gap-1 font-black text-xs ${hasRating ? 'text-amber-500 dark:text-[#A67D44]' : 'text-slate-800 dark:text-[#D4C5B6]'}`}>
                          <Star className={`w-3 h-3 ${hasRating ? 'fill-current' : ''}`} />
                          <span>{ratingDisplay}</span>
                       </div>
                       <p className="text-[8px] font-black text-slate-800 dark:text-[#D4C5B6] uppercase tracking-widest">Đánh giá</p>
                    </div>
                 </div>
               );
            })}
            ...
            {leaderboard.length === 0 && !isLoading && (
              <div className="p-12 text-center text-slate-800 dark:text-[#D4C5B6] text-sm">
