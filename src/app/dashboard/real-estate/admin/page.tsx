...
import {
  ...
  ChevronDown, Search, Trash2, RotateCcw
} from "lucide-react";
...
function ResultBadge({ result }: { result: AuditEntry["result"] }) {
  const cfg = {
    ...
    failure: { icon: XCircle, color: "text-rose-400 bg-rose-500/10" },
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-white/40 text-sm mt-1">Phân quyền, cấu hình, nhật ký và webhook</p>
      </div>
      ...
      {section === "permissions" && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <p className="text-white/40 text-sm">Ma trận phân quyền theo vai trò trong hệ thống Real Estate ERP</p>
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white font-bold">{row.role}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1.5">
                        {row.permissions.map(p => (
                          <span key={p} className="text-xs bg-white/8 border border-white/10 rounded-full px-2.5 py-0.5 text-white/60">
                            {p}
                          </span>
                        ))}
                      </div>
      ...
      {section === "config" && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <p className="text-white/40 text-sm">Cấu hình biến động theo tenant — thay đổi có hiệu lực ngay sau khi lưu</p>
          <div className="space-y-3">
            {configs.map(cfg => (
              <div key={cfg.key} className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <code className="text-xs bg-amber-500/10 text-amber-400 px-2 py-0.5 rounded font-mono">{cfg.key}</code>
                ...
                <div className="sm:w-48 shrink-0">
                  {cfg.type === "boolean" ? (
                    ...
                    <select
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-500/50 cursor-pointer"
                    >
                      {cfg.options?.map(o => <option key={o} value={o} className="bg-slate-900">{o}</option>)}
                    </select>
                  ) : (
                    <input
                      type={cfg.type}
          ...
          <button className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-black px-6 py-2.5 rounded-xl font-bold text-sm transition-all">
          ...
          <div className="space-y-2">
            {filteredAudit.map((entry, i) => (
              <motion.div
                ...
                transition={{ delay: i * 0.05 }}
                className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-start gap-3"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <ResultBadge result={entry.result} />
                    <span className="text-white font-bold text-sm">{entry.action}</span>
                  </div>
                  <p className="text-white/50 text-xs">
                    <span className="text-amber-400 font-semibold">{entry.actor}</span>
                    {" · "}{entry.resource}
                  </p>
                  {entry.details && (
                    <p className="text-rose-400/70 text-xs mt-1 italic">{entry.details}</p>
                  )}
                </div>
            ...
            {filteredAudit.length === 0 && (
              <p className="text-center text-white/20 text-sm italic py-8">Không có nhật ký phù hợp</p>
            )}
          </div>
        </motion.div>
      )}
      ...
      {section === "webhook-dlq" && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-white/40 text-sm">Các webhook thất bại đang chờ retry</p>
          ...
          {MOCK_DLQ.length === 0 && (
            <div className="text-center py-16">
              <CheckCircle2 className="w-12 h-12 mx-auto text-emerald-400/40 mb-3" />
              <p className="text-white/30 text-sm">DLQ trống — tất cả webhook đang hoạt động bình thường ✅</p>
            </div>
          )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <code className="text-xs bg-rose-500/10 text-rose-400 px-2 py-0.5 rounded font-mono">{entry.event}</code>
                    <span className="text-white/30 text-xs">#{entry.id}</span>
                  </div>
                  <p className="text-white/60 text-sm truncate">{entry.endpoint}</p>
                  <p className="text-rose-400/70 text-xs mt-1 italic">{entry.lastError}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 rounded-xl text-xs font-bold transition-all border border-amber-500/20">
                    <RotateCcw className="w-3 h-3" />
                    Retry
                  </button>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/40 rounded-xl text-xs font-bold transition-all border border-white/10">
