...
const renderReadableChanges = (oldData: any, newData: any) => {
  ...
  const ignoredFields = ['id', 'created_at', 'updated_at', 'user_id', 'id_staff'];
  ...
  if (!oldData && newData) return <span className="text-xs font-black text-emerald-500 uppercase tracking-[0.2em] italic">Khởi tạo dữ liệu mới</span>;
  if (oldData && !newData) return <span className="text-xs font-black text-rose-500 uppercase tracking-[0.2em] italic">Đã xóa khỏi hệ thống</span>;
  ...
  if (typeof newData === 'object' && newData !== null) {
    for (const key in newData) {
      ...
      if (oldVal !== newVal) {
        changes.push(
          <div key={key} className="flex items-center gap-4 text-[11px] font-bold py-1 border-b border-slate-50 last:border-none">
            <span className="text-slate-400 w-32 shrink-0">{fieldLabels[key] || key}:</span>
  ...
  return changes.length > 0 ? (
    ...
    <span className="text-xs text-slate-400 italic">Cập nhật thông tin hệ thống</span>
  );
};
        ...
        {activeTab === 'audit' && (
          ...
          >
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h2 className="text-3xl font-black text-foreground uppercase tracking-tight">Nhật ký thay đổi hệ thống</h2>
                <p className="text-sm text-muted-foreground font-semibold">Toàn bộ lịch sử cập nhật và điều chỉnh dữ liệu Bella Spa</p>
              </div>
            </div>
              ...
              <table className="w-full text-left min-w-[800px]">
                <thead>
                  <tr className="border-b border-white/50 bg-white/20">
                    <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">Thời điểm</th>
                    <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">Quản trị viên</th>
                    <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">Phân hệ</th>
                    <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">Chi tiết thay đổi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/20">
                  {isLoadingLogs ? (
                    <tr>
                      <td colSpan={4} className="py-24 text-center">
                        ...
                        <p className="text-slate-500 font-black uppercase tracking-widest text-xs">Đang truy xuất dữ liệu...</p>
                      </td>
                    </tr>
                  ) : auditLogs.length > 0 ? (
                    auditLogs.map((log) => (
                      <tr key={log.id} className="group hover:bg-white/60 transition-all">
                        ...
                        <td className="px-8 py-8">
                          <div className="flex flex-col gap-1.5">
                            ...
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{log.module}</span>
                          </div>
                        </td>
                        <td className="px-8 py-8">
                    ...
                    <tr>
                      <td colSpan={4} className="py-24 text-center">
                        <p className="text-muted-foreground font-bold italic text-lg">Chưa có bản ghi nào được lưu lại</p>
                      </td>
                    </tr>
                  )}
                </tbody>
