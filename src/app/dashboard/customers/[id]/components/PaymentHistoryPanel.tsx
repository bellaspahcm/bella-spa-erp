...
import { calculateConfirmedPaidAmount, calculatePriceAfterDiscount } from '@/lib/business-rules/payment';
...
export function PaymentHistoryPanel({
  ...
  return (
            <div className="bg-white customer-detail-card rounded-[3rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 mt-8">
              ...
              <div className="space-y-4">
                {activeBooking.revenue && activeBooking.revenue.length > 0 ? (
                  activeBooking.revenue.map((rev) => {
                    const isUpfrontFullPayment = rev.revenue_type === 'deposit' && Number(rev.amount) >= priceAfterDiscount;
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="font-black text-slate-800">
                                {isUpfrontFullPayment ? 'Thanh toán trọn gói' :
