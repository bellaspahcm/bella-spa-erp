...
import { User, ShieldAlert, Stethoscope, Armchair, Hash, HeartPulse, ChevronDown } from 'lucide-react';
...
export function PatientContextHeader({
  ...
  return (
    <div className="relative p-5 rounded-[26px] bg-white text-slate-900 shadow-md border border-slate-200/80 overflow-hidden">
      ...
      <div className="relative flex flex-col xl:flex-row xl:items-center justify-between gap-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative inline-block group">
                <select
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-900 font-extrabold text-base pr-8 pl-3 py-1.5 rounded-xl border border-slate-200 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-teal-500/20 transition-all"
                >
                  {allPatients.map((p) => (
                    <option key={p.id} value={p.id} className="bg-white text-slate-950 font-bold text-xs py-2">
                      {p.name} ({p.recordNumber})
                    </option>
                  ))}
                </select>
                <ChevronDown className="w-4 h-4 text-slate-500 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none group-hover:text-slate-900 transition-colors" />
              </div>
        ...
        <div className="flex items-center gap-2">
          {hasAllergies ? (
            <div className="px-4 py-2.5 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 flex items-center gap-3 shadow-sm">
              <ShieldAlert className="w-5 h-5 text-rose-600 shrink-0 animate-bounce" />
              <div className="text-left">
                <p className="text-[10px] font-black uppercase tracking-wider text-rose-600">Cảnh báo Lâm sàng (Allergies)</p>
            ...
            <div className="px-3.5 py-2 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center gap-2 text-xs font-bold shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0 pt-3 xl:pt-0 border-t xl:border-t-0 border-slate-100">
          {/* Doctor Badge */}
          <div className="px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center gap-2">
            <Stethoscope className="w-4 h-4 text-teal-600" />
            <div className="text-left">
              <span className="text-[9px] font-bold text-slate-500 uppercase block">Bác sĩ phụ trách</span>
          ...
          <div className="px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center gap-2">
            ...
            <div className="text-left">
              <span className="text-[9px] font-bold text-slate-500 uppercase block">Ghế điều trị</span>
              <span className="text-xs font-extrabold text-slate-900 leading-tight">{chair?.code || 'Ghế #03 (Khu A)'}</span>
            </div>
          </div>

          {/* Aggregate Code Badge */}
          <div className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-200/60 text-teal-800 flex items-center gap-2">
            <Hash className="w-4 h-4 text-teal-600" />
            <div className="text-left">
              <span className="text-[9px] font-black uppercase text-teal-600 block">Encounter Aggregate</span>
