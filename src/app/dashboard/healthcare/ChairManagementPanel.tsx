...
export interface ChairManagementPanelProps {
  ...
  readonly isMedicalClinic?: boolean;
}
...
export function ChairManagementPanel({
      ...
      case 'occupied':
        return (
          <span className="px-2.5 py-0.5 text-[10px] font-black bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-500/20 rounded-full flex items-center gap-1">
            ...
            {isMedicalClinic ? 'ĐANG KHÁM' : 'ĐANG ĐIỀU TRỊ'}
          </span>
        );
      case 'available':
        return (
  ...
  return (
    <div className="p-6 rounded-[28px] hc-glass-card border border-slate-200/90 dark:border-slate-800/90 shadow-xl space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div>
          <h2 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
            ...
            {isMedicalClinic
              ? 'Quản lý Phòng khám & Tài nguyên Đa khoa (Ma trận Phòng khám)'
              : 'Quản lý Ghế & Tài nguyên điều trị (Ma trận Ghế)'}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            {isMedicalClinic
              ? 'Quản lý và điều phối các phòng khám, công suất bác sĩ và các khu vực lâm sàng theo thời gian thực'
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="px-3.5 py-1.5 rounded-2xl bg-teal-500/10 border border-teal-500/20 text-left">
            <span className="text-[9px] font-bold text-teal-600 dark:text-teal-400 uppercase block">
              {isMedicalClinic ? 'Hiệu suất Phòng khám' : 'Hiệu suất sử dụng Ghế'}
            </span>
      ...
      {/* Chair / Room Cards Matrix Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {chairs.map((chair) => {
          const displayCode = isMedicalClinic ? chair.code.replace('Ghế', 'Phòng khám') : chair.code;
          const displayZone = isMedicalClinic
            ? chair.zone.replace('Ghế chính', 'Phòng Nội khoa').replace('Phục hình', 'Phòng Ngoại khoa')
            : chair.zone;
          ...
          return (
            ...
            >
              ...
              <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 mb-3">{displayZone}</p>
              ...
              {chair.status === 'occupied' ? (
                ...
                <div className="space-y-1.5 pt-2 border-t border-amber-100 dark:border-amber-900/40 text-[10px]">
                  <p className="font-bold text-amber-700 dark:text-amber-300">Đang khử trùng & vệ sinh y tế</p>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 text-center py-2">
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                    {isMedicalClinic ? 'Phòng trống sẵn sàng' : 'Ghế trống sẵn sàng'}
                  </p>
                  {onAssignChair && (
                    <button
                    ...
                    >
                      {isMedicalClinic ? '+ Phân phòng ngay' : '+ Phân ghế ngay'}
                    </button>
                  )}
                </div>
              )}
