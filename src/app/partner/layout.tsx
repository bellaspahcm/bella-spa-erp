...
async function applyPartnerBrandRuntime(tenantSettings?: any) {
  ...
  root.dataset.tenantBrandPreset = brand.stylePreset || 'luxury_navy';

  document.title = `Partner Portal — ${tenant.name}`;
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        // Allowed roles for Partner Portal
        const allowedRoles = ['partner', 'broker', 'agency', 'admin', 'sale'];
        const userRole = user.role?.toLowerCase() || '';
        
        if (!allowedRoles.includes(userRole)) {
          toast.error('Tài khoản của bạn không có quyền truy cập Partner Portal');
          router.replace('/login');
          return;
        }
