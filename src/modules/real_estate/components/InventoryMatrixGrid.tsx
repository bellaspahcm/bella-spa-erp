...
const STATUS_CONFIG: Record<string, StatusConfigItem> = {
  available: {
    ...
    bg: 'bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100 hover:border-emerald-400 dark:hover:bg-emerald-950/50',
  ...
  booked: {
    ...
    bg: 'bg-amber-50 dark:bg-amber-950/30 hover:bg-amber-100 hover:border-amber-400 dark:hover:bg-amber-950/50',
  ...
  deposited: {
    ...
    bg: 'bg-rose-50 dark:bg-rose-950/30 hover:bg-rose-100 hover:border-rose-400 dark:hover:bg-rose-950/50',
  ...
  contracted: {
    ...
    bg: 'bg-purple-50 dark:bg-purple-950/30 hover:bg-purple-100 hover:border-purple-400 dark:hover:bg-purple-950/50',
  ...
  paid: {
    ...
    bg: 'bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 hover:border-blue-400 dark:hover:bg-blue-950/50',
  ...
  handed_over: {
    label: 'Bàn Giao',
    border: 'border-yellow-350 dark:border-yellow-800',
    bg: 'bg-yellow-50 dark:bg-yellow-950/30 hover:bg-yellow-100 hover:border-yellow-400 dark:hover:bg-yellow-950/50',
  ...
  cancelled: {
    ...
    bg: 'bg-slate-50 dark:bg-slate-900/30 hover:bg-slate-100 hover:border-slate-400 dark:hover:bg-slate-900/50',
...
export const InventoryMatrixGrid: React.FC<InventoryMatrixGridProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md">
        <div>
        ...
        <div className="flex flex-wrap gap-2.5">
          {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
            <div
              key={key}
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-xs"
            >
              <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
              <span className="font-bold text-slate-700 dark:text-slate-350">{cfg.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-6">
        {sortedFloors.length === 0 ? (
          <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-400 font-semibold flex flex-col items-center justify-center gap-2 shadow-sm">
          ...
          sortedFloors.map((floor) => {
            ...
            return (
              <div
                key={floor}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-md hover:shadow-lg transition-all duration-300"
              >
                <div className="flex items-center gap-3 mb-4 border-b border-slate-100 dark:border-slate-850 pb-3">
