...
function toDomain(row: OrderRow): ClinicalOrder {
  return ClinicalOrder.fromPersistence({
    ...
    orderType: row.order_type as unknown as ClinicalOrder['orderType'],
    orderStatus: row.order_status as unknown as ClinicalOrder['orderStatus'],
    priority: row.priority as unknown as ClinicalOrder['priority'],
    ...
    cdsCheckStatus: (row.cds_check_status as unknown as ClinicalOrder['cdsCheckStatus']) ?? undefined,
    orderDetails: row.order_details as unknown as ClinicalOrder['orderDetails'],
...
function toInsert(order: ClinicalOrder, requestId?: string): OrderInsert {
  return {
    ...
    order_details: order.orderDetails as unknown as OrderInsert['order_details'],
