  ...
  let query = supabase
    ...
    .select(`
      *, 
      ...
      packages!bookings_package_id_fkey(name),
      session_logs(id, booking_id, session_number, assigned_date, assigned_time, completed_date, start_time, end_time, status, notes, ktv:users!session_logs_completed_by_ktv_id_fkey(full_name))
    `)
    .order('created_at', { ascending: false });
