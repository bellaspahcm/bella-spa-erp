-- =========================================================================
-- Migration: Finance Goods Revenue Semantic Mapping
-- Contract: FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1 extension
-- Purpose:
--   Enable tenant-configured GOODS_REVENUE mapping for goods/product revenue
--   runtime pilot.
-- Scope:
--   - Additive extension to existing read-only semantic map contract.
--   - No inventory, COGS, legacy template/sync/report cutover.
--   - No platform fallback to 5112, 5111, 5113, or any universal account code.
-- =========================================================================
...
LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.control_type NOT IN ('SERVICE_REVENUE', 'REVENUE_DEDUCTION', 'GOODS_REVENUE') THEN
        RETURN NEW;
    END IF;
...
COMMENT ON FUNCTION public.finance_accounting_semantic_map_overlap_guard() IS
    'Guards FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1 determinism for SERVICE_REVENUE, REVENUE_DEDUCTION, and GOODS_REVENUE by rejecting overlapping effective ranges.';
...
COMMENT ON FUNCTION public.finance_get_accounting_semantic_gl_map_as_of(UUID, TEXT, DATE, TEXT) IS
    'FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1. Read-only pilot contract exposing tenant-configured SERVICE_REVENUE, REVENUE_DEDUCTION, and GOODS_REVENUE GL mappings as of a date. No 5111/5112/5113/521 platform fallback.';
