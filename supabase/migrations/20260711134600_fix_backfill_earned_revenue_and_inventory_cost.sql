-- Migration: fix backfill_accounting_metadata to dynamically calculate earned_revenue for sessions and amount for inventory logs
-- This prevents completed sessions and inventory consumptions from being incorrectly flagged as NEEDS_REVIEW during backfill.
...
COMMENT ON FUNCTION public.backfill_accounting_metadata IS
  'Backfills business_event_type and accounting_metadata, dynamically calculating earned_revenue for session_logs and cost for inventory_logs. Skip is_locked=true records for revenue, expenses, salary_records.';
