-- =============================================================================
-- Migration: Add Session Multiplier to Packages & Upgrade Salary Calculations
-- Ngày: 2026-05-30
-- Mục đích:
--   1. Thêm cột session_multiplier vào bảng packages.
--   2. Cập nhật hệ số mẫu cho các gói dịch vụ (VIP = 2.0, Hạnh Phúc = 1.5, Tiết Kiệm/Khác = 1.0).
--   3. Sửa đổi RPC calculate_ktv_salary_sheet để tính tổng số ca theo hệ số quy đổi.
--   4. Cập nhật các RPC đối soát lương đồng bộ kiểu dữ liệu total_sessions mới.
-- =============================================================================

-- 1. Thêm cột session_multiplier vào bảng packages nếu chưa tồn tại
ALTER TABLE public.packages 
ADD COLUMN IF NOT EXISTS session_multiplier NUMERIC(3,2) DEFAULT 1.0;

-- 2. Cấu hình hệ số mặc định cho các gói
UPDATE public.packages 
SET session_multiplier = 2.0 
WHERE name ILIKE '%VIP%';

UPDATE public.packages 
SET session_multiplier = 1.5 
WHERE name ILIKE '%Hạnh Phúc%' OR name ILIKE '%Hanh Phuc%';

UPDATE public.packages 
SET session_multiplier = 1.0 
WHERE session_multiplier IS NULL OR (name NOT ILIKE '%VIP%' AND name NOT ILIKE '%Hạnh Phúc%' AND name NOT ILIKE '%Hanh Phuc%');

-- 3. Drop các hàm liên quan trước khi sửa kiểu dữ liệu return table (để tránh lỗi Postgres signature mismatch)
DROP FUNCTION IF EXISTS public.get_salary_reconciliation(DATE);
...
-- 4. Định nghĩa lại hàm calculate_ktv_salary_sheet hỗ trợ total_sessions kiểu NUMERIC
...
BEGIN
    ...
    completed_sessions AS (
        SELECT s.completed_by_ktv_id AS ktv_id,
               -- Tính tổng số ca làm theo hệ số quy đổi của gói
    ...
    SELECT
        ...
        COALESCE(cs.sessions_count, 0)::NUMERIC AS total_sessions,
...
-- 5. Định nghĩa lại hàm get_salary_reconciliation_report
...
-- 6. Định nghĩa lại hàm get_salary_reconciliation
