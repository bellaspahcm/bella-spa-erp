-- ============================================================================
-- Booking Engine - Database Schema
-- Created: 2026-07-09
...
CREATE TABLE IF NOT EXISTS waitlist (
  ...
  preferred_ktv_id UUID REFERENCES employees(id) ON DELETE SET NULL,
  notes TEXT,
...
-- Function: Get available capacity for time slot
...
BEGIN
  -- Count total KTVs (not on leave, active status)
  ...
  SELECT COUNT(DISTINCT b.assigned_ktv_id) INTO v_booked
  FROM bookings b
  WHERE 
...
-- ============================================================================
-- SEED DATA (Example pricing rules)
-- ============================================================================

-- Note: This will be run by migration, but commented for safety
-- Uncomment and adjust for your tenant

/*
INSERT INTO pricing_rules (tenant_id, rule_name, rule_type, condition, multiplier, priority, enabled) VALUES
  -- Peak hours
  ('YOUR_TENANT_ID', 'Peak Morning', 'peak_hour', '{"hour_range": [10, 12]}'::jsonb, 1.15, 100, true),
  ('YOUR_TENANT_ID', 'Peak Evening', 'peak_hour', '{"hour_range": [18, 21]}'::jsonb, 1.20, 100, true),
  
  -- Weekend
  ('YOUR_TENANT_ID', 'Weekend Premium', 'weekend', '{"days": ["Sat", "Sun"]}'::jsonb, 1.15, 90, true),
  
  -- Advance booking discount
  ('YOUR_TENANT_ID', 'Early Bird', 'advance', '{"days_advance_min": 7}'::jsonb, 0.90, 80, true),
  
  -- Last minute surge
  ('YOUR_TENANT_ID', 'Last Minute', 'advance', '{"days_advance_max": 1}'::jsonb, 1.20, 85, true),
  
  -- High demand surge
  ('YOUR_TENANT_ID', 'High Demand', 'demand', '{"utilization_min": 80}'::jsonb, 1.10, 70, true),
  
  -- Low demand discount
  ('YOUR_TENANT_ID', 'Low Demand Fill', 'demand', '{"utilization_max": 30}'::jsonb, 0.85, 70, true),
  
  -- VIP discount
  ('YOUR_TENANT_ID', 'VIP Discount', 'customer_tier', '{"tier": "vip"}'::jsonb, 0.85, 60, true),
  
  -- New customer promotion
  ('YOUR_TENANT_ID', 'New Customer', 'customer_tier', '{"tier": "new", "first_booking": true}'::jsonb, 0.95, 60, true);
*/
