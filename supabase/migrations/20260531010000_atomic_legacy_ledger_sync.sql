-- Atomic legacy SIMPLE -> PROFESSIONAL ledger sync.
-- The full sync and mode switch run inside one PostgreSQL function call, so
-- any failed insert/update aborts the whole operation instead of leaving a
-- partially posted ledger.
...
BEGIN
    ...
    SELECT id INTO v_revenue_acc
    FROM public.accounting_accounts
    WHERE tenant_id = p_tenant_id AND account_code = '5111' AND is_active = true
    LIMIT 1;
    ...
    IF v_cash_acc IS NULL OR v_bank_acc IS NULL OR v_revenue_acc IS NULL THEN
        RAISE EXCEPTION 'Missing required COA accounts 111, 112 or 5111 for tenant %.', p_tenant_id;
    END IF;
    ...
    LOOP
        ...
        INSERT INTO public.journal_entries (
            ...
            p_tenant_id,
            '[Đồng bộ lịch sử] ' || COALESCE(rec.notes, 'Doanh thu dịch vụ'),
            'PACKAGE_SALE',
            rec.id,
            rec.received_date,
            'DRAFT',
        ...
        VALUES
            ...
            (v_entry_id, v_revenue_acc, 0, v_amount);
    ...
    LOOP
        ...
        INSERT INTO public.journal_entries (
            ...
            p_tenant_id,
            '[Đồng bộ lịch sử] ' || COALESCE(rec.description, 'Chi phí vận hành'),
            'EXPENSE',
            rec.id,
            rec.expense_date,
            'DRAFT',
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            INSERT INTO public.journal_entries (
                ...
                p_tenant_id,
                '[Đồng bộ lịch sử] Hạch toán chi phí lương KTV - Kỳ ' || rec.month_year,
                'SALARY_ACCRUAL',
                rec.id,
        ...
        IF NOT EXISTS (
            ...
            INSERT INTO public.journal_entries (
                ...
                p_tenant_id,
                '[Đồng bộ lịch sử] Chi trả lương KTV - Kỳ ' || rec.month_year,
                'SALARY_PAYMENT',
                rec.id,
                COALESCE(rec.paid_date, rec.month_year::DATE),
                'DRAFT',
