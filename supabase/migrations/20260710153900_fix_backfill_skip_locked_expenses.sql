-- Migration: fix backfill_accounting_metadata to skip is_locked=true records
-- Tables with is_locked column: revenue, expenses, salary_records.
-- The previous version would fail with "Cannot edit a locked record" when
-- trying to UPDATE any of these tables where is_locked = true. Fix: exclude
-- locked records both from the staging INSERT and from the UPDATE statements.
...
BEGIN
    ...
    -- Expenses: SKIP locked records to avoid "Cannot edit a locked record" trigger
    INSERT INTO pg_temp.accounting_backfill_stage (
        source_table, source_id, tenant_id, business_event_type, payload
    )
    SELECT
    ...
    WHERE e.tenant_id = v_tenant_id
      ...
      AND (e.is_locked IS NULL OR e.is_locked = false)   -- ← skip locked expenses
    ...
    -- Apply to expenses (ONLY unlocked records — locked ones were excluded from stage above)
    ...
    WHERE s.source_table = 'expenses'
      ...
      AND (e.is_locked IS NULL OR e.is_locked = false);  -- ← safety guard

    -- Apply to salary_records (safety guard: only update unlocked)
...
COMMENT ON FUNCTION public.backfill_accounting_metadata IS
  'Backfills business_event_type and accounting_metadata for all source tables, skipping is_locked=true records for revenue, expenses, and salary_records. Fixed in 20260710153900.';
