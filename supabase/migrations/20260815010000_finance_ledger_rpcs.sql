-- Migration: finance_ledger_rpcs
-- Description: Creates the transactional database procedures (RPCs) for Ledger posting and reversal.
-- By storing this logic in database functions, we guarantee strict ACID transaction boundaries
-- and prevent any race conditions between parallel posting and period close operations.

-- =========================================================================
-- 1. POST TRANSACTION FUNCTION
-- =========================================================================
...
BEGIN
    ...
    IF v_existing_tx_id IS NOT NULL THEN
        IF v_existing_hash = p_request_hash THEN
            -- Safe duplicate request, return transaction details
    ...
    INSERT INTO public.finance_outbox_events (
        ...
        gen_random_uuid(), p_tenant_id, v_event_id, 'finance.transaction.posted.v1', v_outbox_payload, 'PENDING', NOW()
    );
...
-- =========================================================================
-- 2. REVERSE TRANSACTION FUNCTION
-- =========================================================================
...
DECLARE
    ...
    v_line RECORD;
BEGIN
    -- 1. Check if reversal is already posted (idempotency key check)
    ...
    SELECT status, reversal_of, posted_at, transaction_currency, functional_currency,
           exchange_rate_rate, exchange_rate_source, exchange_rate_target, exchange_rate_effective,
           description, reference_type, reference_id, accounting_period_id
    INTO v_orig_status, v_orig_reversal_of, v_orig_posted_at, v_orig_tx_currency, v_orig_func_currency,
         ...
         v_orig_desc, v_orig_ref_type, v_orig_ref_id, v_period_id
    FROM public.finance_transactions
    WHERE tenant_id = p_tenant_id
    ...
    -- Verify period is still open (reversal is posted in the original period or current)
    ...
    -- 3. Update original transaction status to REVERSED
    UPDATE public.finance_transactions
    SET status = 'REVERSED',
        updated_at = NOW()
    WHERE id = p_transaction_id;

    -- 4. Create Reversal Transaction Header
    ...
    INSERT INTO public.finance_transactions (
        ...
        v_period_id, NOW(), v_orig_tx_currency, v_orig_func_currency,
        v_orig_rate, v_orig_rate_src, v_orig_rate_tgt, v_orig_rate_eff,
        'Reversal of: ' || v_orig_desc || ' (Reason: ' || p_reason || ')', v_orig_ref_type, v_orig_ref_id, p_transaction_id, NOW(), NOW()
    );

    -- 5. Copy lines with swapped debit and credit
    FOR v_line IN 
    ...
    INSERT INTO public.finance_audit_trail (
        ...
        jsonb_build_object('id', p_transaction_id, 'status', 'REVERSED', 'reversal_id', v_reversal_tx_id), NOW()
    );

    -- 7. Insert outbox events for both the state update and reversal creation
    v_event_id := gen_random_uuid();
    v_outbox_payload := jsonb_build_object(
...
-- Grants for the functions to authenticated users
GRANT EXECUTE ON FUNCTION public.finance_post_transaction TO authenticated;
GRANT EXECUTE ON FUNCTION public.finance_reverse_transaction TO authenticated;
