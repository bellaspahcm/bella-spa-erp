-- ============================================================
-- Fix Rule Management RPCs - Column Name Corrections
-- ============================================================
-- Issue: RPC functions reference u.name, u1.name, u2.name
-- Fix: Change to u.full_name, u1.full_name, u2.full_name
-- Issue: rule_versions.change_type check constraint missing 'updated'
-- Fix: Add 'updated' to allowed values
-- ============================================================
...
DO $$
BEGIN
  RAISE NOTICE '✅ Rule Management RPC fixes applied successfully';
  RAISE NOTICE '   - get_rule_with_history: u.name → u.full_name';
  RAISE NOTICE '   - get_pending_rule_approvals: u1.name, u2.name → full_name';
  RAISE NOTICE '   - rule_versions: Added "updated" to change_type constraint';
  RAISE NOTICE '   - rules: Added "archived" to status constraint';
END $$;
