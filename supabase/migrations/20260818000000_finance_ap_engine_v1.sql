...
BEGIN
    ...
    WHERE tenant_id = p_tenant_id
      ...
      AND is_active = TRUE
      AND (account_type = p_expected_type OR p_expected_type IS NULL);
...
BEGIN
    ...
    -- Post to F1 GL (Debit Prepayment Asset / Credit Bank)
    v_f1_tx_id := public.finance_post_transaction(
        p_tenant_id, 'CASH', NOW(),
        'AP_PREPAYMENT_RECORDED: Vendor ' || p_vendor_id::TEXT,
        p_posting_attempt_id::TEXT,
        jsonb_build_array(
            jsonb_build_object('account_code', 'PREPAYMENT_ASSET', 'direction', 'DEBIT',  'amount_minor', p_amount_minor),
            jsonb_build_object('account_id',   p_bank_finance_account_id::TEXT, 'direction', 'CREDIT', 'amount_minor', p_amount_minor)
        )
    );
...
BEGIN
    ...
    -- Post to F1 GL (Debit AP Control 331 / Credit Prepayment Asset)
    v_f1_tx_id := public.finance_post_transaction(
        p_tenant_id, 'ACCRUAL', NOW(),
        'AP_PREPAYMENT_APPLIED: Bill ' || p_bill_id::TEXT,
        p_posting_attempt_id::TEXT,
        jsonb_build_array(
            jsonb_build_object('account_code', '331',              'direction', 'DEBIT',  'amount_minor', p_amount_minor),
            jsonb_build_object('account_code', 'PREPAYMENT_ASSET', 'direction', 'CREDIT', 'amount_minor', p_amount_minor)
        )
    );
