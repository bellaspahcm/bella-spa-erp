-- Migration: Fix backfill_accounting_metadata to default payment_method to 'bank_transfer' for inventory purchase logs
-- This prevents purchase/restock logs from being incorrectly flagged as MISSING_REQUIRED_FIELDS (due to missing payment_method) during backfill.

CREATE OR REPLACE FUNCTION public.backfill_accounting_metadata(
  p_tenant_id uuid DEFAULT NULL::uuid,
  p_limit integer DEFAULT 500
)
RETURNS TABLE(source_table text, scanned_records integer, classified_records integer, review_created integer)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE r.tenant_id = v_tenant_id
      ...
      AND (r.is_locked IS NULL OR r.is_locked = false)
    ORDER BY r.received_date DESC, r.id
    LIMIT v_limit;

    -- Expenses: SKIP locked records
    INSERT INTO pg_temp.accounting_backfill_stage (
        source_table, source_id, tenant_id, business_event_type, payload
    )
    SELECT
    ...
    WHERE e.tenant_id = v_tenant_id
      ...
      AND (e.is_locked IS NULL OR e.is_locked = false)
    ORDER BY e.expense_date DESC, e.id
    LIMIT v_limit;

    -- Salary records: skip locked
    INSERT INTO pg_temp.accounting_backfill_stage (
        source_table, source_id, tenant_id, business_event_type, payload
    )
    SELECT
    ...
    WHERE s.tenant_id = v_tenant_id
      ...
      AND (s.is_locked IS NULL OR s.is_locked = false)
    ORDER BY s.month_year DESC, s.id
    LIMIT v_limit;

    -- Session logs: LEFT JOIN bookings to dynamically compute earned_revenue
    INSERT INTO pg_temp.accounting_backfill_stage (
        source_table, source_id, tenant_id, business_event_type, payload
    )
    SELECT
    ...
    -- Inventory logs: LEFT JOIN inventory_items to dynamically compute amount cost + default payment_method for purchases
    INSERT INTO pg_temp.accounting_backfill_stage (
        source_table, source_id, tenant_id, business_event_type, payload
    )
    SELECT
        ...
        END,
        jsonb_strip_nulls(jsonb_build_object(
            'amount', ABS(il.change_amount) * COALESCE(ii.price_per_unit, 0),
            'payment_method', CASE
                WHEN lower(COALESCE(il.reason, '')) IN ('purchase', 'import', 'stock_in', 'restock') THEN 'bank_transfer'
                ELSE NULL
            END,
            'item_id', il.item_id,
    ...
    LEFT JOIN public.inventory_items ii ON ii.id = il.item_id
    ...
    -- Mark missing fields and needs_review
    ...
    -- Apply to revenue (safety guard: only update unlocked)
    ...
    WHERE s.source_table = 'salary_records'
      ...
      AND (sr.is_locked IS NULL OR sr.is_locked = false);

    -- Apply to session_logs
    ...
    -- Queue items that need manual review
...
COMMENT ON FUNCTION public.backfill_accounting_metadata IS
  'Backfills business_event_type and accounting_metadata, dynamically calculating earned_revenue for session_logs and cost for inventory_logs (defaulting payment_method = bank_transfer for restock events). Skip is_locked=true records for revenue, expenses, salary_records.';
