-- Keep salary reconciliation semantics aligned across AI Copilot and
-- accounting report views. Missing saved salary records are workflow-pending,
...
BEGIN
    ...
    SELECT
        ...
        CASE
            WHEN lr.legacy_tot IS NULL THEN NULL
            ELSE (COALESCE(lr.legacy_tot, 0) - ai.ai_tot)::NUMERIC
        END AS diff_total,
        CASE
            WHEN lr.legacy_tot IS NULL THEN NULL
        ...
        CASE
            ...
            WHEN ai.ai_tot > 0
                ...
                THEN 'MATCH'
            WHEN ai.ai_tot > 0
                 AND ABS(COALESCE(lr.legacy_tot, 0) - ai.ai_tot) / ai.ai_tot < 0.05
                THEN 'MINOR_DIFF'
            ELSE 'MAJOR_DIFF'
        END::TEXT AS status
    FROM ai_sheet ai
