-- Scope Legacy-vs-Ledger reconciliation to journals that originate from legacy finance rows.
-- Professional runtime journals are valid ledger activity, but they do not have matching
-- legacy finance rows and must not be reported as drift in this migration-check report.
...
BEGIN
    ...
    WHERE e.tenant_id = p_tenant_id
      AND e.status = 'POSTED'
      AND e.reference_type IN ('REVENUE', 'REFUND', 'PACKAGE_SALE')
    ...
    WHERE e.tenant_id = p_tenant_id
      AND e.status = 'POSTED'
      AND e.reference_type IN ('EXPENSE', 'SALARY_ACCRUAL')
      AND e.entry_date::DATE BETWEEN p_from_date AND p_to_date
...
COMMENT ON FUNCTION public.get_reconciliation_report(UUID, DATE, DATE)
IS 'Compares TT133-recognized legacy finance amounts with ledger entries that originate from legacy finance rows, excluding professional runtime-only journals.';
