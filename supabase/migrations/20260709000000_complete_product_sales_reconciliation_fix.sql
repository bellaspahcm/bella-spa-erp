-- =============================================================================
-- Migration: Complete product sales commission reconciliation fix
-- Date: 2026-07-09
-- Purpose:
--   Update get_salary_reconciliation_report RPC to include product_sales_commission
--   in both legacy and AI columns for complete reconciliation display.
--
-- Background:
--   Migration 20260623000000 updated calculate_ktv_salary_sheet but missed
--   get_salary_reconciliation_report, causing UI to not display product sales.
-- =============================================================================
...
DECLARE
    v_period_month DATE;
BEGIN
    -- Set tenant context for this session (required for nested RPC calls)
    PERFORM public.set_session_tenant(p_tenant_id);
...
GRANT EXECUTE ON FUNCTION public.get_salary_reconciliation_report(UUID, DATE) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_salary_reconciliation_report(UUID, DATE) TO service_role;

-- Force PostgREST to reload schema
NOTIFY pgrst, 'reload schema';

-- Add comment for documentation
COMMENT ON FUNCTION public.get_salary_reconciliation_report(UUID, DATE) IS 
'Returns salary reconciliation report comparing legacy (manually entered) salary records 
with AI-calculated salary sheet. Includes product_sales_commission in both columns.
Updated: 2026-07-09 to complete product sales commission integration.';
