-- =========================================================================
-- Migration: Finance Revenue Deduction Semantic Mapping
-- Contract: FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1 extension
-- Purpose:
--   Enable tenant-configured REVENUE_DEDUCTION mapping for refund/revenue
--   reversal runtime pilot.
-- Scope:
--   - Additive extension to existing read-only semantic map contract.
--   - No legacy template/sync/report cutover.
--   - No platform fallback to 521, 5113, 5111, or any universal account code.
-- =========================================================================
...
LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.control_type NOT IN ('SERVICE_REVENUE', 'REVENUE_DEDUCTION') THEN
        RETURN NEW;
    END IF;
...
COMMENT ON FUNCTION public.finance_accounting_semantic_map_overlap_guard() IS
    'Guards FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1 determinism for SERVICE_REVENUE and REVENUE_DEDUCTION by rejecting overlapping effective ranges.';
...
BEGIN
    ...
    IF p_semantic_key NOT IN ('SERVICE_REVENUE', 'REVENUE_DEDUCTION') THEN
...
COMMENT ON FUNCTION public.finance_get_accounting_semantic_gl_map_as_of(UUID, TEXT, DATE, TEXT) IS
    'FINANCE_ACCOUNTING_SEMANTIC_GL_MAP:v1. Read-only pilot contract exposing tenant-configured SERVICE_REVENUE and REVENUE_DEDUCTION GL mappings as of a date. No 5111/5113/521 platform fallback.';
