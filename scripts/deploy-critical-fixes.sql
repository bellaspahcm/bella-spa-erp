...
\echo '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
\echo 'PART 1: FIX RLS INFINITE RECURSION'
\echo '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
...
DECLARE
  v_policy RECORD;
BEGIN
  RAISE NOTICE 'Backing up existing user_roles policies...';
  FOR v_policy IN 
...
DO $$
BEGIN
  RAISE NOTICE 'Dropping problematic policies...';
  
  DROP POLICY IF EXISTS "Admins can view all roles" ON user_roles;
  RAISE NOTICE '  ✓ Dropped: Admins can view all roles';
  
  DROP POLICY IF EXISTS "Super admins can manage roles" ON user_roles;
  RAISE NOTICE '  ✓ Dropped: Super admins can manage roles';
  
  DROP POLICY IF EXISTS "Only service role can modify roles" ON user_roles;
  RAISE NOTICE '  ✓ Dropped: Only service role can modify roles (if exists)';
  
  DROP POLICY IF EXISTS "Authenticated users can view all roles" ON user_roles;
  RAISE NOTICE '  ✓ Dropped: Authenticated users can view all roles (if exists)';
END $$;
...
BEGIN
  ...
  CREATE POLICY "Authenticated users can view all roles"
    ...
    TO authenticated
    USING (true);
  RAISE NOTICE '  ✓ Created: Authenticated users can view all roles';
  ...
  CREATE POLICY "Only service role can modify roles"
    ...
    USING (false)
    WITH CHECK (false);
  RAISE NOTICE '  ✓ Created: Only service role can modify roles';
  
  -- Grant necessary permissions
  GRANT SELECT ON user_roles TO authenticated;
  RAISE NOTICE '  ✓ Granted: SELECT permission to authenticated';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Verification: % policies found on user_roles', v_policy_count;
  
  IF v_policy_count >= 2 THEN
    RAISE NOTICE '✅ RLS Fix deployed successfully!';
  ELSE
...
\echo ''
\echo '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
\echo 'PART 2: DEPLOY PARTNER PORTAL'
\echo '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'partner_applications') THEN
    RAISE NOTICE '⚠️  partner_applications already exists - skipping table creation';
  ELSE
    RAISE NOTICE 'Creating partner portal tables...';
  END IF;
END $$;
...
DO $$ BEGIN
  ...
  RAISE NOTICE '  ✓ Created enum: partner_application_status';
EXCEPTION WHEN duplicate_object THEN 
  RAISE NOTICE '  ⚠️  Enum already exists: partner_application_status';
END $$;

DO $$ BEGIN
  CREATE TYPE partner_applicant_type AS ENUM ('individual', 'company');
  RAISE NOTICE '  ✓ Created enum: partner_applicant_type';
EXCEPTION WHEN duplicate_object THEN
  RAISE NOTICE '  ⚠️  Enum already exists: partner_applicant_type';
END $$;
...
DO $$ BEGIN
  ...
  RAISE NOTICE '  ✓ Created enum: partner_application_log_action';
EXCEPTION WHEN duplicate_object THEN
  RAISE NOTICE '  ⚠️  Enum already exists: partner_application_log_action';
END $$;
...
RAISE NOTICE '  ✓ Created tables and indexes';
...
BEGIN
  ...
  RAISE NOTICE '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━';
  RAISE NOTICE 'Part 1: RLS Fix - % policies deployed', v_policies;
  RAISE NOTICE 'Part 2: Partner Portal - Tables, policies, and % RPC functions deployed', v_funcs;
  RAISE NOTICE '';
  RAISE NOTICE '📋 Next Steps:';
  RAISE NOTICE '  1. Test admin login (should not see infinite recursion)';
  RAISE NOTICE '  2. Test partner registration at /partner/register';
