/**
 * COMPLETE CLEANUP - Delete ALL massage-related entries and recreate correctly
 * 
 * This will:
  ...
  for (const entry of entries) {
    ...
    const { data: reversalEntry, error: createError } = await supabase
      ...
      .insert({
        tenant_id: tenantId,
        entry_date: today,
        description: `[FINAL CLEANUP] Đảo toàn bộ massage entries - ${entry.id.substring(0, 8)}`,
...
async function main() {
  try {
    ...
    console.log('FINAL COMPLETE CLEANUP - MASSAGE ACCOUNTING');
    console.log('='.repeat(80));
    ...
    if (shouldFix) {
      ...
      rl.question('\n⚠️  This will REVERSE ALL massage entries and create NEW correct ones.\nProceed? (yes/no): ', async (answer: string) => {
      ...
      console.log('\nℹ️  Run with --fix flag to execute the final cleanup.');
      console.log('   Example: npx tsx scripts/complete-cleanup-massage.ts --fix');
    }
  } catch (error) {
