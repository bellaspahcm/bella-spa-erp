-- ============================================
-- Task 10 Test Data Setup Script (Simplified)
-- ============================================
...
DO $$
BEGIN
  -- Step 1: Delete booking_service_items (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_service_items') THEN
    DELETE FROM public.booking_service_items 
    WHERE booking_id IN (
      SELECT id FROM public.bookings 
      WHERE booking_number LIKE 'BK-BEAUTY-TEST-%'
    );
  END IF;

  -- Step 2: Delete session_logs
  ...
  WHERE email IN ('admin.beauty@test.com', 'ktv1.beauty@test.com', 'ktv2.beauty@test.com');
...
SELECT 
  session_logs.id, 
  session_logs.booking_id, 
  session_logs.session_number,
  session_logs.status,
...
-- ============================================
SELECT 
  'Setup completed! Access these URLs:' as message
UNION ALL
SELECT '/dashboard/bookings/99999999-9999-9999-9999-999999999999/services'
UNION ALL
SELECT 'Login: admin.beauty@test.com';
