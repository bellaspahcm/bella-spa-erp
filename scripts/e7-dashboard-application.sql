-- ============================================================================
-- E7 Migration Dashboard Application Script
-- Date: 2026-09-03
-- Target: bella-spa-erp-e2e (dev database)
-- Reason: Supabase CLI timeout - manual application via Dashboard SQL Editor
-- ============================================================================
-- IMPORTANT: Run this ENTIRE script in Supabase Dashboard SQL Editor
-- ============================================================================
...
DO $$
BEGIN
  -- Check if logistics schema already exists
  IF EXISTS (
    SELECT 1 FROM information_schema.schemata WHERE schema_name = 'logistics'
  ) THEN
    RAISE EXCEPTION 'ERROR: logistics schema already exists! Do not run this script.';
  END IF;
  
  -- Check if E7 migration already recorded
  IF EXISTS (
    SELECT 1 FROM supabase_migrations.schema_migrations 
    WHERE version = '20260822000000'
  ) THEN
    RAISE EXCEPTION 'ERROR: E7 migration already recorded in history! Do not run this script.';
  END IF;
  
  RAISE NOTICE 'Pre-flight checks passed. Proceeding with E7 migration...';
END $$;

-- ============================================================================
-- STEP 2: APPLY E7 MIGRATION CONTENT
-- ============================================================================
...
VALUES (
  ...
  ARRAY['-- See supabase/migrations/20260822_logistics_os_domain_kernel.sql for full content']
);
...
BEGIN
  ...
  IF schema_count = 0 THEN
    RAISE EXCEPTION 'ERROR: logistics schema was not created!';
  END IF;
  ...
  IF table_count != 6 THEN
    RAISE EXCEPTION 'ERROR: Expected 6 tables, found %', table_count;
  END IF;
  ...
  IF migration_count = 0 THEN
    RAISE EXCEPTION 'ERROR: Migration was not recorded in schema_migrations!';
  END IF;
  
  RAISE NOTICE '✅ E7 migration applied successfully!';
  RAISE NOTICE '✅ logistics schema created';
  RAISE NOTICE '✅ 6 tables created: items, locations, inventory, inventory_movements, traceability, uom';
  RAISE NOTICE '✅ Migration recorded in history';
END $$;
...
SELECT 
  '✅ E7 Migration Complete' AS status,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'logistics') AS table_count,
  (SELECT array_agg(table_name ORDER BY table_name) FROM information_schema.tables WHERE table_schema = 'logistics') AS tables;

-- ============================================================================
-- END OF E7 DASHBOARD APPLICATION SCRIPT
-- ============================================================================
