...
console.log(`Starting k6 Cloud Run using AWS Singapore zone for ${scriptPath}...`);

// Construct command
const cmd = `k6 cloud run ` +
  `-e BASE_URL="${BASE_URL}" ` +
  `-e SUPABASE_URL="${SUPABASE_URL}" ` +
  `-e SUPABASE_ANON_KEY="${SUPABASE_ANON}" ` +
  `-e SUPABASE_SERVICE_KEY="${SERVICE_KEY}" ` +
  `-e ENVIRONMENT="singapore-cloud" ` +
  `"${scriptPath}"`;

try {
  execSync(cmd, { stdio: 'inherit' });
} catch (error) {
  console.error('k6 cloud run execution finished with error.');
  process.exit(1);
}
