  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === 'get_financial_performance') {
        ...
        if (revErr) throw new Error(`Lỗi doanh thu: ${revErr.message}`);
        ...
        if (expErr) throw new Error(`Lỗi chi phí: ${expErr.message}`);
        ...
        if (salErr) throw new Error(`Lỗi quỹ lương: ${salErr.message}`);
        ...
        const revenueBreakdown = (revenueData || []).reduce((acc: any, r) => {
          acc[r.revenue_type] = (acc[r.revenue_type] || 0) + Number(r.amount);
          return acc;
        }, {});

        const expenseBreakdown = (expenseData || []).reduce((acc: any, e) => {
      ...
      if (name === 'calculate_salary_projection') {
        ...
        if (ktvErr) throw new Error(ktvErr.message);

        const wageForecasts: any[] = [];
      ...
      if (name === 'inspect_customer_treatment_progress') {
        ...
        const cust = booking.customers as any;
        const analysis = {
          booking_details: {
      ...
      if (name === 'propose_zalo_reminders') {
        ...
        const proposals = (sessions || []).map(s => {
          const bookings = s.bookings as any;
          const customer = bookings?.customers as any;
