...
* **Data/action isolation**: Fetch sessions, bookings, KTV, session history và Supabase realtime subscription đã được gom vào `src/app/dashboard/bookings/hooks/useBookingsPageData.ts`; QR payment, update session, create schedule được gom vào `src/app/dashboard/bookings/hooks/useBookingsPageActions.ts`. Page chính chỉ còn điều phối UI/state.
* **Session action split**: `createSessionLog` và `rescheduleSession` đã được tách khỏi `session-mutation-actions.ts` sang `create-session-log-action.ts` và `reschedule-session-action.ts`; wrapper công khai trong `session-actions.ts` giữ nguyên để không đổi import ở UI.
* **Update session action hardening**: `updateSessionLog` đã tách helper `update-session-log-helpers.ts` cho normalize payload, completion defaults, completion side-effects và booking progress sync; payload update dùng type `session_logs.Update`, không còn `any` ở action public.
* **Create booking action split**: `createBooking` đã tách `create-booking-helpers.ts` cho rate limit, tạo customer, resolve tenant, upsert booking, ghi revenue cọc/accounting outbox và tạo session logs ban đầu; DB payload dùng type `bookings.Insert/Update`, `revenue.Insert`, `session_logs.Insert`.
* **Session completion engine split**: `processSessionCompletion` đã tách `session-completion-helpers.ts` cho kiểm tra kỳ kế toán, trừ/rollback kho, sync booking progress, tạo revenue gói lẻ/outbox, sync lương KTV, review placeholder và SESSION_DONE outbox; engine chính chỉ còn điều phối flow.
* **Payment action split**: `recordRemainingPayment`, `generateShareToken`, `getBookingDetailsWithPayment` đã tách `payment-helpers.ts` cho snapshot booking, validate overpayment, kiểm tra khóa kỳ, gọi RPC thanh toán, update share token và fetch QR/payment detail.
* **Salary recalculation engine split**: `recalculateAndSaveSalaryRecord` trong `admin-salary-actions.ts` giữ wrapper public, còn engine tính lương đã tách sang `salary-recalculation-engine.ts` để gom pro-rata, session multiplier, KPI sync, rating bonus, deduction và preserve non-draft logic.
* **Salary admin workflow split**: Khóa kỳ lương, tạo salary expense/accounting review, audit status và revalidate dashboard đã tách sang `admin-salary-workflow-helpers.ts`; các action publish/confirm/finalize/approve/update/confirm sessions giữ nguyên API.
* **Pure helpers & explicit UI state**: Các helper lịch/thời gian/modal được gom vào `src/app/dashboard/bookings/utils/bookingsPageUtils.ts`; thao tác tạo lịch từ timeline truyền `createDate` bằng state/props rõ ràng thay vì đọc DOM bằng `document.querySelector`.
...
* **Kiểm tra bắt buộc sau refactor**: Các bước refactor đã chạy `npx.cmd tsc --noEmit` và `npx.cmd eslint src/app/dashboard/bookings/page.tsx src/app/dashboard/bookings/components/*.tsx src/app/dashboard/bookings/hooks/*.ts src/app/dashboard/bookings/utils/*.ts` pass trước khi commit/push.
* **Commit nổi bật**: `9e4446e`, `c0b2a23`, `6d03fe8`, `5d641bd`, `0a21a4f`, `3035176`, `aa8f4e1`, `c6d505f`, `39a3f4f`, `b46e245`, `79d0411`, `1e0eda0`, `43d37a7`, `1694f4e` cho booking refactor; `e8fc839`, `eaa074b` cho KTV dashboard refactor.
